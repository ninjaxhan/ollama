...
func caseingShuffle(s string) string {
	rr := []rune(s)
	for i := range rr {
		if rand.N(1) == 0 {
...
func TestCase(t *testing.T) {
	...
	checkManifestList := func() {
		...
		const want = "example/namespace/model/tag"
		if g := got[0]; !strings.EqualFold(g, want) {
			t.Errorf("EqualFold(%v, %v) != true", g, want)
		}
	}
